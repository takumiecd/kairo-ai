...
- **段階A**: `decode/beam.py` でのスコア融合。母集団のデフォルト分布を基準に、
  プロファイル語彙の頻度・recency・明示採用/却下を候補スコア差分として加える。
- **段階B（効果検証後）**: 全profileで共有する一つのモデルについて、デフォルト経路と
  profile差分経路を同時に学習する。profileごとのLoRAや個別モデルは作らない。
  旧profile-conditioned RNN-Tは比較対象の実験実装として扱う。
