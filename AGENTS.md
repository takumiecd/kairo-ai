...
A trained checkpoint (`rnnt-trf-v1`: Transformer encoder 4L + predictor 2L, 256-dim, ~7.6M params, valid CER 5.87% at epoch 20) is published at https://huggingface.co/takumiecd/rnnt-trf-v1. `hf download takumiecd/rnnt-trf-v1 --local-dir artifacts/rnnt-trf-v1` gives a ready artifact dir for the decoders. An artifact dir needs `config.json` + `input_vocab.json` + `output_vocab.json` + a checkpoint; checkpoints also embed a copy of the training config under the `"config"` key, so `config.json` can be regenerated from one if lost.
